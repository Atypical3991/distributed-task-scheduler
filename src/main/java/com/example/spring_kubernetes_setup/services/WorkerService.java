...
import com.example.spring_kubernetes_setup.listeners.AssignmentListener;
import com.example.spring_kubernetes_setup.listeners.JobsListener;
import com.example.spring_kubernetes_setup.listeners.WorkersListener;
import com.example.spring_kubernetes_setup.modules.strategy.RoundRobinWorker;
import com.example.spring_kubernetes_setup.modules.strategy.WorkerPickerStrategy;
...
public class WorkerService implements LeaderSelectorListener, Closeable {
    ...
    private void registerWorker() {
        if (registrationRequired.get()) {
            ...
            asyncCreate(ZKUtils.getAssignmentPath(name), CreateMode.PERSISTENT, null);
            watchAssignmentPath();
            // irrespective of whether this node is a leader or not, we need to create and watch the
            // assignment path
        }
    }
